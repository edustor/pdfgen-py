...
def make_pdf(page_count, output_file, x_cells=40, y_cells=56, cell_side=5 * mm):
    ...
    for n in range(page_count):
        ...
        c.drawRightString(x_max, bottom_row_y, "#{}-{}-{}".format(page_id[-12:-8], page_id[-8:-4], page_id[-4:]))

        now = datetime.now(pytz.timezone("Europe/Moscow"))
        now_str = now.strftime("%a %b %d %X %Z %Y")
        copyright_string = "Generated by Edustor PDFG on {} ".format(now_str) + \
                           "© Edustor Project. Dmitry Romanov, 2016-{}".format(now.year)
        c.drawString(x_min, bottom_row_y, copyright_string)

        qr_code_side = 3 * cell_side
        _draw_qr_code("https://edustor.ru/p/{}".format(page_id), c,
